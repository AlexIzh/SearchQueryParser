...
enum Token<Item: Operator> {
	case bracket(Bool)//is opening bracket or not
	case value(String)
	case `operator`(Item)
}
...
class Tokenizer<Item: Operator> {
	...
	func tokenize(_ string: String) -> [Token<Item>] {
		...
		for (index, current) in tokens.enumerated() {
			...
			case .default:
				if let item = operatorItem(for: current.value) {
					if item.type == .binary {
						...
						if next != nil {
							if prev.map(isOperator(_:)) == false {
			...
			case .phrase:
				if prev.map(isClosingBrackets(_:)) == true {
					result.append(.operator(whitespace))
				} else if prev.map(isOperator(_:)) == false {
					result.append(.operator(whitespace))
				}
			...
			case .validClosingBrackets:
				result.append(.bracket(false))
				
			case .validOpeningBrackets:
				result.append(.bracket(true))
			}
		}
